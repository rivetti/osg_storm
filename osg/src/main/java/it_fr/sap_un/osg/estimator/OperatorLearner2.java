...
public class OperatorLearner2 implements IOperatorLearner {
	...
	public OperatorLearner2(int opId, double delta, double epsilon, long seed,
	...
	private class FSM {
		...
		private SnapShot prevEstimatorSnapshot;
		...
		public void newTuple(ITuple tuple, long latency) {
			...
			// Update Estimator
			int value = OperatorLearner2.this.hasher.hash(tuple.getValues()
					.get(OperatorLearner2.this.attributeId));
			...
			if (data != null && data.estimatedTerminationTs > 0) {
				...
				System.out.println(String.format("sched: %d, op: %d (%d)",
						data.estimatedTerminationTs,
		...
		private class Stabilizing extends State {
			...
			public State newTuple(ITuple tuple, long latency) {
				...
				if (m % OperatorLearner2.this.windowSize == 0) {
					...
					if (error <= TOLLERANCE) {
						...
						System.out
								.println(String
										.format("[%02d] OperatorLearner [%d::%d] Estimator Sabilized [%f <= %f ]",
												...
												TOLLERANCE));
						if (FSM.this.prevEstimatorSnapshot == null) {

							System.out
									.println(String
											.format("[%02d] OperatorLearner [%d::%d] First Stabilization",
													opId, FSM.this.m,
													FSM.this.window));
							...
							double changes = FSM.this.estimator
									.error(FSM.this.prevEstimatorSnapshot);

							if (changes <= TOLLERANCE) {
								System.out
										.println(String
												.format("[%02d] OperatorLearner [%d::%d] No changes detected [%f <= %f ]",
														opId, FSM.this.m,
														FSM.this.window,
														changes, TOLLERANCE));
								...
								System.out
										.println(String
												.format("[%02d] OperatorLearner [%d::%d] Changes detected [%f > %f ]",
														opId, FSM.this.m,
														FSM.this.window,
														changes, TOLLERANCE));
						...
						FSM.this.prevEstimatorSnapshot = FSM.this.estimator
								.getSnapShot();
